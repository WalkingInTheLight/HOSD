...
class OperaTask(BaseTask):
    ...
    def run_model(self, model, sample, return_output=False, infer=False):
        ...
        self.add_mel_loss(output['diff_mel'], target, losses)
    ...
    def validation_step(self, sample, batch_idx):
        ...
        if batch_idx < hparams['num_valid_plots']:
            ...
            if hparams.get('pe_enable') is not None and hparams['pe_enable']:
                ...
                fs2_f0 = self.pe(model_out['mel_out'])['f0_denorm_pred']  # pe predict from Pred mel
            else:
                gt_f0 = denorm_f0(sample['f0'], sample['uv'], hparams)
                pred_uv = model_out['uv']
                diff_f0 = denorm_f0(model_out['diff_f0'], pred_uv, hparams)

            self.plot_wav(batch_idx, sample['mels'], model_out['diff_mel'], is_mel=True,
                          gt_f0=gt_f0, diff_f0=diff_f0)
            self.plot_mel(batch_idx, sample['mels'], model_out['diff_mel'], name=f'diffmel')
            self.plot_f0(batch_idx, gt_f0, diff_f0, name=f'diff_f0')
        return outputs
    ...
    def add_f0_loss(self, p_pred, f0, uv, losses, nonpadding):
        # print(p_pred[..., 0].shape, f0.shape)
        assert p_pred[..., 0].shape == f0.shape
        if hparams['use_uv']:
            assert p_pred[..., 1].shape == uv.shape
            # print("p_pred[:, :, 1]:",p_pred[:, :, 1])
            # print('uv: ', uv)
            losses['uv'] = (F.binary_cross_entropy_with_logits(
                p_pred[:, :, 1], uv, reduction='none') * nonpadding).sum() \
                           / nonpadding.sum() * hparams['lambda_uv']
